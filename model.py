...
class MaskedEdgeAttention(nn.Module):
    ...
    def forward(self, M, lengths, edge_ind):
        ...
        if attn_type == 'attn1':
            ...
            device = M.device  # Ensures compatibility with model device
            ...
            if len(edge_ind_) > 0:  # Ensure edge_ind_ is not empty
                edge_ind_ = torch.tensor(edge_ind_, dtype=torch.long, device=device).T  # Convert to tensor and transpose
            ...
            scores = masked_alpha.div(_sums + 1e-10) * mask_copy  # Prevent division by zero
            return scores
