...
class DialogueGCNModel(nn.Module):
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future,
        ...
        if self.base_model == 'DialogRNN':
            ...
            raise ValueError('Base model must be one of DialogRNN/LSTM/GRU/None')

        # Add context enhancer
        self.context_enhancer = ContextEnhancer(2*D_e)  # 2*D_e because of bidirectional

        n_relations = 2 * n_speakers ** 2
        self.window_past = window_past
        self.window_future = window_future

        self.att_model = AdaptiveTemporalAttention(2*D_e, max_seq_len, self.no_cuda)
    ...
    def _reverse_seq(self, X, mask):
        """Reverse sequence according to mask"""
        X_ = X.transpose(0,1)
    ...
    def forward(self, U, qmask, umask, seq_lengths):
        ...
        # Apply context enhancement for short utterances
        g_hist = emotions.mean(dim=0, keepdim=True)  # Global context
        enhanced_emotions = self.context_enhancer(emotions, g_hist.expand_as(emotions))

        features, edge_index, edge_norm, edge_type, edge_index_lengths = batch_graphify(
            enhanced_emotions, qmask, seq_lengths, 
        ...
        log_prob = self.graph_net(
            ...
            seq_lengths, umask, self.nodal_attention, self.avec
        )
...
# Perspective 1 : Mécanisme d’Attention Statique → Dynamique pour calculer le  poids des aretes 
class AdaptiveTemporalAttention(nn.Module):
    def __init__(self, input_dim, max_seq_len, no_cuda):
        """
        Mécanisme d'attention temporelle adaptative avec gestion correcte des arêtes.
        """
        ...
        self.score_projection = nn.Linear(input_dim, 1)
        
    def compute_adaptive_scores(self, attn_output, edge_ind):
        """
        Version corrigée du calcul des scores.
        edge_ind est maintenant une liste plate de tuples (src, tgt)
        """
        seq_len, batch_size, _ = attn_output.size()
        
        # Initialisation des scores
        scores = torch.zeros(len(edge_ind))
        if not self.no_cuda:
            scores = scores.cuda()
            
        # Calcul des scores pour chaque arête
        for i, (src, tgt) in enumerate(edge_ind):
            if src < seq_len and tgt < seq_len:
                # Calcul du score de similarité
                score = torch.dot(attn_output[src,0,:], attn_output[tgt,0,:])
                scores[i] = score
            else:
                scores[i] = -1e10  # Valeur très basse pour les arêtes invalides
    ...
    def forward(self, M, lengths, edge_ind):
        """
        M -> (seq_len, batch, input_dim)
        lengths -> longueurs des séquences
        edge_ind -> liste de tuples (src, tgt) représentant les arêtes
        """
        # Encodage positionnel
        M = self.pos_encoder(M)
        
        # Masque de padding
        src_key_padding_mask = torch.zeros(M.size(1), M.size(0)).bool()
        ...
        # Attention multi-têtes
        attn_output, _ = self.time_attn(
            M, M, M,
            key_padding_mask=src_key_padding_mask
        )
        
        # Calcul des scores adaptatifs
        scores = self.compute_adaptive_scores(attn_output, edge_ind)
        
        return scores

class PositionalEncoding(nn.Module):
    """
    Module d'encodage positionnel pour injecter des informations temporelles.
    """
...
# Perspective 2 : Adaptive Temporal Attention Mechanism Integration pour  les fenetres 

class WindowPredictor(nn.Module):
    def __init__(self, input_dim, hidden_dim=32, max_window=10):
        super(WindowPredictor, self).__init__()
        self.max_window = max_window
        self.mlp = nn.Sequential(
            nn.Linear(input_dim * 2, hidden_dim),  # Prend deux états en entrée
            nn.ReLU(),
            nn.Linear(hidden_dim, 1),
            nn.Sigmoid())  # Sortie normalisée [0,1]

    def forward(self, current_state, other_state):
        """
        current_state: (batch, D_e) - état courant
        other_state: (batch, D_e) - état à comparer
        Retourne: (batch,) window sizes (int)
        """
        combined = torch.cat([current_state, other_state], dim=-1)
        window_ratio = self.mlp(combined).squeeze(-1)
        return (1 + (self.max_window - 1) * window_ratio).int()

def dynamic_edge_perms(seq_len, current_idx, features, window_predictor, max_window=10):
    """
    features: (seq_len, batch, D_e)
    """
    perms = set()
    ...
    for t in range(seq_len):
        ...
        # Calcul des fenêtres pour tout le batch en une passe
        window_sizes = window_predictor(
            features[current_idx],  # (batch, D_e)
            features[t]             # (batch, D_e)
        )
        
        # Génération des connexions
        for b in range(batch_size):
            if abs(t - current_idx) <= window_sizes[b]:
                perms.add((current_idx, t))
                perms.add((t, current_idx))
    
    return list(perms)


def batch_graphify(features, qmask, lengths, window_past, window_future, 
                  edge_type_mapping, att_model, no_cuda):
    edge_index, edge_norm, edge_type = [], [], []
    batch_size = features.size(1)
    length_sum = 0
    batch_perms = []  # To store perms for each batch item
    
    # Initialize window predictor
    window_predictor = WindowPredictor(
        input_dim=features.size(-1),
        max_window=max(window_past, window_future)
    )
    
    # Build graph for each batch item
    for j in range(batch_size):
        seq_len = int(lengths[j])
        node_features = features[:seq_len, j]
        
        # Generate adaptive edges
        perms = []
        for t in range(seq_len):
            perms.extend(dynamic_edge_perms(
                seq_len, t, 
                features[:, j:j+1],  # (seq_len, 1, D_e)
                window_predictor
            ))
        
        batch_perms.append(perms)  # Store perms for this batch item
        length_sum += seq_len
    
    # Compute attention scores for all edges at once
    if len(batch_perms) > 0:
        # Flatten all edges across batch
        all_perms = []
        batch_idx = []
        offset = 0
        for j, perms in enumerate(batch_perms):
            all_perms.extend([(t+offset, s+offset) for (t,s) in perms])
            batch_idx.extend([j] * len(perms))
            offset += int(lengths[j])
        
        if len(all_perms) > 0:
            # Get all node features concatenated
            all_features = torch.cat([features[:lengths[j], j] for j in range(batch_size)])
            scores = att_model(all_features.unsqueeze(0), torch.tensor(lengths), all_perms)
            
            # Distribute scores back to each batch item
            score_ptr = 0
            for j, perms in enumerate(batch_perms):
                if len(perms) > 0:
                    batch_scores = scores[score_ptr:score_ptr+len(perms)]
                    score_ptr += len(perms)
                    
                    # Convert to global indices and store
                    offset = sum(lengths[:j])
                    for (t,s), score in zip(perms, batch_scores):
                        edge_index.append(torch.tensor([t+offset, s+offset]))
                        edge_norm.append(score)
                        
                        # Determine edge type
                        speaker_src = qmask[t,j].argmax().item()
                        speaker_tgt = qmask[s,j].argmax().item()
                        direction = '0' if t < s else '1'
                        edge_type.append(edge_type_mapping[f"{speaker_src}{speaker_tgt}{direction}"])

    # Final conversion
    edge_index = torch.stack(edge_index).t() if edge_index else torch.empty(2, 0).long()
    edge_norm = torch.stack(edge_norm) if edge_norm else torch.empty(0)
    edge_type = torch.tensor(edge_type) if edge_type else torch.empty(0).long()
    ...
    features = torch.cat([features[:lengths[j], j] for j in range(batch_size)])
    
    return features, edge_index, edge_norm, edge_type, [len(edge_index)]
...
# Perspective 3 :  Context  Enhancer 
