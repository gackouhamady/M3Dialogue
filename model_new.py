...
class AdaptiveTemporalAttention(nn.Module):
    def __init__(self, input_dim, max_seq_len, no_cuda):
        """
        Mécanisme d'attention temporelle adaptative remplaçant le MaskedEdgeAttention statique.
        Utilise une attention multi-têtes pour calculer dynamiquement les poids des arêtes.
        """
        ...
        # Attention multi-têtes avec 4 têtes pour une meilleure capacité d'apprentissage
        ...
        # Encodage positionnel pour capturer la temporalité
        self.pos_encoder = PositionalEncoding(input_dim)
        
        # Couche de projection pour adapter la dimension des scores
        self.score_projection = nn.Linear(input_dim, 1)
        
    def compute_adaptive_scores(self, attn_output, edge_ind):
        """
        Calcule les scores d'attention adaptatifs en fonction des connexions du graphe.
        """
        seq_len, batch_size, _ = attn_output.size()
        
        # Initialisation du tenseur de scores avec la bonne shape
        scores = torch.zeros(batch_size, self.max_seq_len, self.max_seq_len)
        if not self.no_cuda:
            scores = scores.cuda()
            
        # Projection des features
        projected = self.score_projection(attn_output).squeeze(-1)  # (seq_len, batch)
        projected = projected.permute(1, 0)  # (batch, seq_len)
        
        for j in range(batch_size):
            # Initialisation des masques pour cet échantillon
            sample_mask = torch.ones(self.max_seq_len, self.max_seq_len) * 1e-10
            mask_copy = torch.zeros(self.max_seq_len, self.max_seq_len)
            
            if not self.no_cuda:
                sample_mask = sample_mask.cuda()
                mask_copy = mask_copy.cuda()
            
            # Remplir les connexions existantes
            for (src, tgt) in edge_ind[j]:
                if src < seq_len and tgt < seq_len:
                    # Calcul du score de similarité
                    score = torch.dot(attn_output[src,j,:], attn_output[tgt,j,:])
                    sample_mask[src, tgt] = score  # Modification ici
                    mask_copy[src, tgt] = 1
            
            # Normalisation des scores
            masked_scores = sample_mask * mask_copy
            sums = masked_scores.sum(-1, keepdim=True)
            scores[j] = masked_scores.div(sums + 1e-10) * mask_copy
    ...
    def forward(self, M, lengths, edge_ind):
        ...
        lengths -> longueurs des séquences dans le batch
        edge_ind -> indices des arêtes pour chaque échantillon
        """
        # Encodage positionnel
        M = self.pos_encoder(M)  # (seq_len, batch, input_dim)
